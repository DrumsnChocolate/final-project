...
for d in "cbis 2 finetune 224 1"; do
  ...
  if [ ${transfer_type} == "finetune" ]; then
    CUDA_VISIBLE_DEVICES=0 sbatch -J "vpt-vit-mammo-${dataset}" --constraint=rtx-6000 slurm/visual_prompt_tuning/vit_mammo.sbatch ${dataset} ${num_classes} ${transfer_type} ${crop_size} ${batch_size}
  else
